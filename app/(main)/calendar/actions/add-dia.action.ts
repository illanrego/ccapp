...
export async function addShow(formData: FormData) {
    try {
        ...
        const freeTickets = formData.get('freeTickets') ? parseInt(formData.get('freeTickets') as string) : 0;
        ...
        const result = await addShowService(showData, comicsWithPositions);
